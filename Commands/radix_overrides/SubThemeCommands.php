...
class SubThemeCommands extends DrushCommands {
  ...
  /**
   * Function to copy config files from radix_overrides to the new theme.
   *
   * @param string $themeName
   *   The name of the newly created theme.
   */
  private function copyConfigFiles(string $themeName) {
    $filesystem = new Filesystem();
    $drupalRoot = Drush::bootstrapManager()->getRoot();
    $source = $drupalRoot . '/themes/shared/radix_overrides/config';
    $destination = $drupalRoot . '/themes/custom/' . $themeName . '/config';

    // Check if config directory exists in radix_overrides.
    if (!$filesystem->exists($source)) {
      return;
    }

    // Remove any config files that were generated by the starterkit
    // to ensure we use the ones from radix_overrides.
    if ($filesystem->exists($destination)) {
      $filesystem->remove($destination);
    }

    // Copy the entire config directory from radix_overrides.
    $filesystem->mirror($source, $destination);

    // Replace all occurrences of 'radix_overrides' with the new theme name
    // in all YAML files within the config directory.
    ...
    foreach ($iterator as $file) {
      if ($file->isFile() && $file->getExtension() === 'yml') {
        ...
        $content = str_replace('radix_overrides', $themeName, $content);
